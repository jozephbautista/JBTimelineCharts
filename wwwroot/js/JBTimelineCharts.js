        ...
        for (var e = 0; e < timelineData.length; e++) {
            if (monthsArray[m] == timelineData[e].month) {
                ...
                if (count <= p.maxDataPointPerMonth) {
                    ...
                    if (timelineData[e].eventtype == eventtypes.THIRDPARTYEVENT) {
                        CTX.fillStyle = p.thridPartyEventFontColor;
                        wrapText(CTX, timelineData[e].title, (p.timelineBarX + (p.timeLineBarMonthWidth * m)) + narrowMonthEdgePadding + (MonthDataPointOffsetX * count) + 10, (p.timelineBarY - lineLength) - 2, p.datapointTextWrapLimit, p.datapointTitleLineHeight, true);
                    }
                    else if (timelineData[e].eventtype == eventtypes.RELEASEMILESTONE) {
                        CTX.fillStyle = p.releaseMilestoneFontColor;
